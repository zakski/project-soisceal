...
public class ThreadLibraryTestCase {
	...
	alice.tuprolog.core.Prolog engine = null;
	String theory;
	
	@Before
	public void before() {
		try {
			engine = new alice.tuprolog.core.Prolog();
	...
	public void testThread_id_1() throws InvalidTheoryException, MalformedGoalException, NoSolutionException {
		...
		assertEquals(new alice.tuprolog.core.data.numeric.Int(0), id);
	}
	...
	public void testThread_read_2() throws InvalidTheoryException, MalformedGoalException, NoSolutionException {
		...
		//Solution sinfo = engine.solve("thread_create(genitore(bob,X), ID), thread_create(genitore(b,Y), ID2), thread_read(ID2,Y), thread_read(ID,X1), thread_next_sol(ID), thread_read(ID,X).");
	...
	public void testMsg_queue_size_2() throws InvalidTheoryException, MalformedGoalException, NoSolutionException {
		...
		assertEquals(new alice.tuprolog.core.data.numeric.Int(5), X);
	}
    ...
    public void testFattoriale() throws InvalidTheoryException, MalformedGoalException, NoSolutionException{
            ...
            Term X = sinfo.getVarValue("X");
            assertEquals(new alice.tuprolog.core.data.numeric.Int(5040), X);
            
            Term Y = sinfo.getVarValue("Y");
            assertEquals(new alice.tuprolog.core.data.numeric.Int(40320), Y);
    }
