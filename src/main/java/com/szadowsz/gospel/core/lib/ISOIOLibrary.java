...
public class ISOIOLibrary extends Library {
	...
	public boolean open_4(Term source_sink, Term mode, Term stream, Term options) throws PrologException {
		initLibrary();
		source_sink = source_sink.getTerm();
		mode = mode.getTerm();

		if (source_sink instanceof Var) {
			throw PrologException.instantiation_error(engine.getEngineManager(), 1);
		}

		File file = new File(((Struct) source_sink).getName());
		if (!file.exists()) {
			throw PrologException.existence_error(engine.getEngineManager(), 1, "source_sink", source_sink, new Struct("File not found."));
		}

		if (mode instanceof Var) {
			throw PrologException.instantiation_error(engine.getEngineManager(), 2);
		} else if (!mode.isAtom()) {
			throw PrologException.type_error(engine.getEngineManager(), 1, "atom", mode);
		}

		if (!(stream instanceof Var)) {
			throw PrologException.type_error(engine.getEngineManager(), 3, "variable", stream);
		}
		...
		if (result == true) {
			...
			if (openOptions.isList()) {
				if (!openOptions.isEmptyList()) {
					Iterator<? extends Term> i = openOptions.iterator();
					while (i.hasNext()) {
						Struct option = null;
						Object obj = i.next();
						if (obj instanceof Var) {
							throw PrologException.instantiation_error(engine.getEngineManager(), 4);
						}
						option = (Struct) obj;
						if (!properties.containsKey(option.getName())) {
							throw PrologException.domain_error(engine.getEngineManager(), 4, "stream_option", option);
						}
											...
											if (((Struct) alias).getArity() > 1) {
												for (int z = 0; z < ((Struct) alias).getArity(); z++) {
													if ((((Struct) alias).getArg(z)).equals(option.getArg(k))) {
														throw PrologException.permission_error(engine.getEngineManager(), "open", "source_sink",
																alias, new Struct("Alias is already associated with an open stream."));
													}
												}
											} else if (alias.equals(option.getArg(k))) {
												throw PrologException.permission_error(engine.getEngineManager(), "open", "source_sink", alias,
														new Struct("Alias is already associated with an open stream."));
											}
										}
									}
								}
				...
				throw PrologException.type_error(engine.getEngineManager(), 4, "list", openOptions);
			}
			...
			if (structMode.getName().equals("write")) {
				try {
					...
					throw PrologException.permission_error(engine.getEngineManager(), "open", "source_sink", source_sink, new Struct(
				...
				if (((Struct) properties.get("reposition")).getName().equals("true")) {
					try {
						...
						try {
							...
							throw PrologException.system_error(new Struct("An error has occurred in open when closing the input file."));
						}
						// END ED
						throw PrologException.system_error(new Struct("An error has occurred in open."));
					}
				}
				...
				throw PrologException.domain_error(engine.getEngineManager(), 2, "io_mode", mode);
			}
		} else {
			PrologException.system_error(new Struct("A problem has occurred with initialization of properties' hashmap."));
			return false;
		}
	}

	public boolean open_3(Term source_sink, Term mode, Term stream) throws PrologException {
		initLibrary();
		...
		if (result == true) {
			...
			if (structMode.getName().equals("write")) {
				...
				throw PrologException.domain_error(engine.getEngineManager(), 1, "stream", in_out);
			}
		} else {
			PrologException.system_error(new Struct("A problem has occurred with the initialization of the hashmap properties."));
			return false;
		}
	}

	public boolean close_2(Term stream_or_alias, Term closeOptions) throws PrologException {
		initLibrary();
		// Struct result = null;
		...
		if (closeOptions.isList()) {
			if (!closeOptions.isEmptyList()) {
				Iterator<? extends Term> i = closeOption.iterator();
				while (i.hasNext()) {
					Struct option = null;
					Object obj = i.next();
					if (obj instanceof Var) {
					...
					if (option.getName().equals("force")) {
						...
						throw PrologException.domain_error(engine.getEngineManager(), 2, "close_option", option);
					}
				}
			}
		} else {
			throw PrologException.type_error(engine.getEngineManager(), 4, "list", closeOptions);
		}
		...
		if (out != null) {
			...
			try {
				...
				if (force == true) {// devo forzare la chiusura
					...
					throw PrologException.system_error(new Struct("An error has occurred on stream closure."));
				}
			}
		} else if (in != null) {
	...
	public boolean close_1(Term stream_or_alias) throws PrologException {
		initLibrary();
		// Struct result = null;
		OutputStream out = null;
		InputStream in = null;
	...
	public boolean set_input_1(Term stream_or_alias) throws PrologException {
		initLibrary();
	...
	public boolean set_output_1(Term stream_or_alias) throws PrologException {
		initLibrary();
	...
	// che la soddisfano
	public boolean find_property_2(Term list, Term property) throws PrologException {
		initLibrary();
	...
	public boolean at_end_of_stream_0() throws PrologException {
		initLibrary();
	...
	public boolean at_end_of_stream_1(Term stream_or_alias) throws PrologException {
		initLibrary();
		InputStream stream = find_input_stream(stream_or_alias);
		Hashtable<String, Term> entry = inputStreams.get(stream);
		Term value = entry.get("end_of_stream");
	...
	public boolean set_stream_position_2(Term stream_or_alias, Term position) throws PrologException {
		...
		if (position instanceof Var) {
			...
			if (!(position instanceof com.szadowsz.gospel.core.data.numeric.Number)) {
				throw PrologException.domain_error(engine.getEngineManager(), 2, "stream_position", position);
			}
		}
		...
		if (value.getName().equals("false")) {
			throw PrologException.permission_error(engine.getEngineManager(), "reposition", "stream", stream_or_alias, new Struct(
		...
		if (buffer.markSupported()) {
			try {
				...
				if (pos < 0) {
					throw PrologException.domain_error(engine.getEngineManager(), 2, "+long", position);
				}
				...
				if (pos > size) {
					throw PrologException.system_error(new Struct("Invalid operation. Input position is greater than file size."));
				}
				if (pos == size) {
				...
				e.printStackTrace();
				throw PrologException.system_error(new Struct("An error has occurred in method 'set_stream_position'."));
			}
		}
		return true;
	}

	public boolean flush_output_0() throws PrologException {
		initLibrary();
		try {
			outputStream.flush();
		} catch (IOException e) {
			throw PrologException.system_error(new Struct("An error has occurred in method 'flush_output_0'."));
		}
		return true;
	}

	public boolean flush_output_1(Term stream_or_alias) throws PrologException {
		initLibrary();
		OutputStream stream = find_output_stream(stream_or_alias);
		try {
			stream.flush();
		} catch (IOException e) {
			throw PrologException.system_error(new Struct("An error has occurred in method 'flush_output_1'."));
		}
		return true;
	}

	public boolean get_char_2(Term stream_or_alias, Term arg) throws PrologException {
		initLibrary();
		InputStream stream = find_input_stream(stream_or_alias);
		Character c = null;
		int value = 0;
		...
		if (type.getName().equals("binary")) {
			throw PrologException.permission_error(engine.getEngineManager(), "input", "binary_stream", stream_or_alias, new Struct(
		...
		// stdin lo scrivo come stringa, non posso usare inputStreamName, perche' in quel campo ci deve rimanere lo stream corrente, e se e' stato
		// cambiato, non ho piu' sdtin
		...
		try {
			...
			if ((eof.getName()).equals("past")) {
				...
				if (action.equals("error"))
					throw PrologException.permission_error(engine.getEngineManager(), "input", "past_end_of_stream", new Struct("reader"),
							new Struct("End of file is reached."));
			...
			if (!Character.isDefined(value)) {
				if (value == -1) {
					...
					throw PrologException.representation_error(engine.getEngineManager(), 2, "character");
				}
			}
			Int i = (Int) position;
			int i2 = i.intValue();
			...
			ioe.printStackTrace();
			throw PrologException.system_error(new Struct("An I/O error has occurred"));
		}
	}

	public boolean get_code_1(Term char_code) throws PrologException {
		initLibrary();
		Struct s_or_a = new Struct(inputStream.toString());
		return get_code_2(s_or_a, char_code);
	}

	public boolean get_code_2(Term stream_or_alias, Term char_code) throws PrologException {
		initLibrary();
		InputStream stream = find_input_stream(stream_or_alias);
		...
		if (file_name.equals("stdin")) {
			try {
				...
				throw PrologException.permission_error(engine.getEngineManager(), "input", "stream", new Struct(inputStreamName),
	...
	public boolean peek_char_1(Term in_char) throws PrologException {
		initLibrary();
		Struct s_or_a = new Struct(inputStream.toString());
		if (inputStreamName.equals("stdin")) {
			...
			try {
				...
				// TODO Auto-generated catch block
				e.printStackTrace();
				PrologException.system_error(new Struct("An error has occurred in peek_char_1."));
			}
			return var;
		} else {
	...
	public boolean peek_char_2(Term stream_or_alias, Term in_char) throws PrologException {
		...
		try {
			...
			e.printStackTrace();
			PrologException.system_error(new Struct("File not found."));
		}
		Character c = null;
		int value = 0;
		...
		try {
			...
			if (eof.equals("past")) {
				...
				if (action.equals("error")) {
					throw PrologException.permission_error(engine.getEngineManager(), "input", "past_end_of_stream", new Struct("reader"),
							new Struct("End of file has been reached."));
				} else if (action.equals("eof_code")) {
	...
	public boolean peek_code_1(Term char_code) throws PrologException {
		initLibrary();
		...
		if (inputStreamName.equals("stdin")) {
			...
			try {
				...
				// TODO Auto-generated catch block
				e.printStackTrace();
				PrologException.system_error(new Struct("An error has occurred in peek_code_1."));
			}
			return var;
		} else {
	...
	public boolean peek_code_2(Term stream_or_alias, Term char_code) throws PrologException {
		initLibrary();
		// come la get_char soltanto non cambio la posizione di lettura
		InputStream stream = find_input_stream(stream_or_alias);
		Hashtable<String, Term> element = (Hashtable<String, Term>) inputStreams.get(stream);
	...
	public boolean put_char_2(Term stream_or_alias, Term in_char) throws PrologException {
		initLibrary();
		OutputStream stream = find_output_stream(stream_or_alias);
		...
		if (!arg0.isAtom()) {
			throw PrologException.type_error(engine.getEngineManager(), 2, "character", arg0);
		} else {
			...
			if (ch.length() > 1) {
				throw PrologException.type_error(engine.getEngineManager(), 2, "character", new Struct(ch));
			} else {
	...
	public boolean put_code_1(Term char_code) throws PrologException {
		initLibrary();
		Struct stream = new Struct(outputStream.toString());
		return put_code_2(stream, char_code);
	}

	public boolean put_code_2(Term stream_or_alias, Term char_code) throws PrologException {
		initLibrary();
		OutputStream stream = find_output_stream(stream_or_alias);
		String stream_name = get_output_name(stream);
		...
		if (Character.isDefined(arg0.intValue())) {
			throw PrologException.representation_error(engine.getEngineManager(), 2, "character_code");
		}
		if (stream_name.equals("stdout")) {
	...
	public boolean nl_1(Term stream_or_alias) throws PrologException {
		initLibrary();
		OutputStream stream = find_output_stream(stream_or_alias);
		String stream_name = get_output_name(stream);
		if (stream_name.equals("stdout")) {
			...
			try {
				...
				throw PrologException.permission_error(engine.getEngineManager(), "output", "stream", new Struct(outputStreamName),
						new Struct(e.getMessage()));
			}
		}
		return true;
	...
	public boolean get_byte_1(Term in_byte) throws PrologException {
	...
	public boolean get_byte_2(Term stream_or_alias, Term in_byte) throws PrologException {
		initLibrary();
		InputStream stream = find_input_stream(stream_or_alias);
		Byte b = null;
		Hashtable<String, Term> element = (Hashtable<String, Term>) inputStreams.get(stream);
		...
		if (type.getName().equals("text")) {
			throw PrologException.permission_error(engine.getEngineManager(), "input", "text_stream", stream_or_alias, new Struct(
	...
	public boolean peek_byte_1(Term in_byte) throws PrologException {
		initLibrary();
		Struct stream_or_alias = new Struct(inputStream.toString());
		return peek_char_2(stream_or_alias, in_byte);
	}

	public boolean peek_byte_2(Term stream_or_alias, Term in_byte) throws PrologException {
		initLibrary();
		InputStream stream = find_input_stream(stream_or_alias);
		Byte b = null;
		Hashtable<String, Term> element = (Hashtable<String, Term>) inputStreams.get(stream);
	...
	public boolean put_byte_1(Term out_byte) throws PrologException {
	...
	public boolean put_byte_2(Term stream_or_alias, Term out_byte) throws PrologException {
		initLibrary();
		OutputStream stream = find_output_stream(stream_or_alias);
		out_byte = out_byte.getTerm();
		...
		Struct type = (Struct) element.get("type");
		if (type.getName().equals("text")) {
			throw PrologException.permission_error(engine.getEngineManager(), "output", "text_stream", stream_or_alias, new Struct(
					"Target stream is associated with a text stream."));
		}
	...
	public boolean read_term_2(Term in_term, Term options) throws PrologException {
		initLibrary();
		Struct stream_or_alias = new Struct(inputStream.toString());
		return read_term_3(stream_or_alias, in_term, options);
	}

	public boolean read_term_3(Term stream_or_alias, Term in_term, Term options) throws PrologException {
		initLibrary();
		InputStream stream = find_input_stream(stream_or_alias);

		if (options instanceof Var) {
			throw PrologException.instantiation_error(engine.getEngineManager(), 3);
		}
		...
		if ((eof.getName()).equals("past") && (action.getName()).equals("error")) {
			throw PrologException.permission_error(engine.getEngineManager(), "past_end_of_stream", "stream", stream_or_alias, new Struct(
		...
		if (readOptions.isList()) {
			if (!readOptions.isEmptyList()) {
				Iterator<? extends Term> i = readOptions.iterator();
				while (i.hasNext()) {
					Struct option = null;
					Object obj = i.next();
					if (obj instanceof Var) {
					...
					if (option.getName().equals("variables")) {
						...
						PrologException.domain_error(engine.getEngineManager(), 3, "read_option", option);
					}
				}
			}
		} else {
			throw PrologException.type_error(engine.getEngineManager(), 3, "list", options);
		}
	...
	public boolean read_2(Term stream_or_alias, Term in_term) throws PrologException {
		initLibrary();
		Struct options = new Struct(".", new Struct());
		return read_term_3(stream_or_alias, in_term, options);
	}

	public boolean write_term_2(Term out_term, Term options) throws PrologException {
		initLibrary();
		Struct stream_or_alias = new Struct(outputStream.toString());
		return write_term_3(stream_or_alias, out_term, options);
	}

	public boolean write_term_3(Term stream_or_alias, Term out_term, Term optionsTerm) throws PrologException {
		initLibrary();
		...
		Struct type = (Struct) element.get("type");
		if (type.getName().equals("binary")) {
			throw PrologException.permission_error(engine.getEngineManager(), "output", "binary_stream", stream_or_alias, new Struct(
					"Target stream is associated with a binary stream."));
		}

		if (writeOptionsList.isList()) {
			if (!writeOptionsList.isEmptyList()) {
				Iterator<? extends Term> i = writeOptionsList.iterator();
				while (i.hasNext()) {
					...
					if (writeOption.getName().equals("quoted")) {
						...
						throw PrologException.domain_error(engine.getEngineManager(), 3, "write_options", writeOptionsList.getTerm());
					}
				}
			}
		} else {
			PrologException.type_error(engine.getEngineManager(), 3, "list", writeOptionsList);
		}
		try {
			...
			PrologException.system_error(new Struct("Write error has occurred in write_term/3."));
		}
		return true;
	}
	...
	public boolean write_2(Term stream_or_alias, Term out_term) throws PrologException {
		initLibrary();
		Struct options = new Struct(".", new Struct("quoted", new Struct("false")), new Struct(".", new Struct("ignore_ops", new Struct("false")),
				new Struct(".", new Struct("numbervars", new Struct("true")), new Struct())));
		return write_term_3(stream_or_alias, out_term, options);
	}

	public boolean write_1(Term out_term) throws PrologException {
		if (write_flag == 0) {
	...
	public boolean write_iso_1(Term out_term) throws PrologException {
		initLibrary();
		Struct stream_or_alias = new Struct(outputStream.toString());
		Struct options = new Struct(".", new Struct("quoted", new Struct("false")), new Struct(".", new Struct("ignore_ops", new Struct("false")),
				new Struct(".", new Struct("numbervars", new Struct("true")), new Struct())));
	...
	public boolean writeq_1(Term out_term) throws PrologException {
		initLibrary();
		Struct stream_or_alias = new Struct(outputStream.toString());
		Struct options = new Struct(".", new Struct("quoted", new Struct("true")), new Struct(".", new Struct("ignore_ops", new Struct("false")),
				new Struct(".", new Struct("numbervars", new Struct("true")), new Struct())));
		return write_term_3(stream_or_alias, out_term, options);
	}

	public boolean writeq_2(Term stream_or_alias, Term out_term) throws PrologException {
		initLibrary();
		Struct options = new Struct(".", new Struct("quoted", new Struct("true")), new Struct(".", new Struct("ignore_ops", new Struct("false")),
				new Struct(".", new Struct("numbervars", new Struct("true")), new Struct())));
		return write_term_3(stream_or_alias, out_term, options);
	...
	public boolean write_canonical_1(Term out_term) throws PrologException {
		initLibrary();
		Struct stream_or_alias = new Struct(outputStream.toString());
		Struct options = new Struct(".", new Struct("quoted", new Struct("true")), new Struct(".", new Struct("ignore_ops", new Struct("true")),
				new Struct(".", new Struct("numbervars", new Struct("false")), new Struct())));
		return write_term_3(stream_or_alias, out_term, options);
	}

	public boolean write_canonical_2(Term stream_or_alias, Term out_term) throws PrologException {
		initLibrary();
		Struct options = new Struct(".", new Struct("quoted", new Struct("true")), new Struct(".", new Struct("ignore_ops", new Struct("true")),
				new Struct(".", new Struct("numbervars", new Struct("false")), new Struct())));
		return write_term_3(stream_or_alias, out_term, options);
	...
	private void initLibrary() {
		...
		library = engine.getLibrary("com.szadowsz.gospel.core.lib.IOLibrary");
		if (library == null) {
			try {
				library = engine.loadLibrary("com.szadowsz.gospel.core.lib.IOLibrary");
			} catch (InvalidLibraryException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				PrologException.system_error(new Struct("IOLibrary does not exists."));
			}
		}
	...
	private InputStream find_input_stream(Term stream_or_alias) throws PrologException {
		int flag = 0;
		...
		if (stream_name.contains("Output") || stream_name.equals("stdout"))
			throw PrologException.permission_error(engine.getEngineManager(), "output", "stream", stream_or_alias, new Struct(
					"S_or_a is an output stream"));

		if (flag == 0)
			// se lo stream non si trova all'interno della hashtable, significa che non ? mai stato aperto
			throw PrologException.existence_error(engine.getEngineManager(), 1, "stream", stream_or_alias, new Struct(
					"Input stream should be opened."));

		return result;
	}

	// stessa funzione di find_input_stream, ma sugli stream di output
	private OutputStream find_output_stream(Term stream_or_alias) throws PrologException {
		int flag = 0;
		...
		if (stream_name.contains("Input") || stream_name.equals("stdin"))
			throw PrologException.permission_error(engine.getEngineManager(), "input", "stream", stream_or_alias, new Struct(
					"S_or_a is an input stream."));

		if (flag == 0)
			// se lo stream non si trova all'interno della hashtable, significa che non ? mai stato aperto
			throw PrologException.existence_error(engine.getEngineManager(), 1, "stream", stream_or_alias, new Struct(
					"Output stream should be opened."));
	...
	public boolean set_write_flag_1(Term number) throws PrologException {
		...
		if (n.intValue() == 1) {
			...
			throw PrologException.domain_error(engine.getEngineManager(), 1, "0-1", number);
		}
	}
}
