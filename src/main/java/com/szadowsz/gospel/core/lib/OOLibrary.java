...
import com.szadowsz.gospel.core.data.numeric.Float;
...
public class OOLibrary extends Library {
    ...
    public void onSolveBegin(Term goal) {
        currentObjects.clear();
        currentObjects_inverse.clear();
        java.util.Iterator<Map.Entry<Object,Struct>> it = staticObjects_inverse.entrySet().iterator();
        while (it.hasNext()) {
     ...
     */
    public boolean java_object_3(Term className, Term argl, Term id) throws JVMException {
     ...
     * @throws JVMException
     */
    public boolean new_object_3(Term className, Term argl, Term id) throws JVMException {
        className = className.getTerm();
        Struct arg = (Struct) argl.getTerm();
        id = id.getTerm();
        try {
            if (!className.isAtom()) {
                throw new JVMException(new ClassNotFoundException(
            ...
            if (clName.endsWith("[]")) {
                ...
                int nargs = ((com.szadowsz.gospel.core.data.numeric.Number) list[0]).intValue();
            ...
            try {
                ...
                if (co == null) {
                   _logger.warn("Constructor not found: class {}",clName);
               ...
               _logger.warn("Constructor not found: {}",args.getTypes());
                throw new JVMException(ex);
            } catch (InstantiationException ex) {
               _logger.warn("Objects of class {} cannot be instantiated",clName);
                throw new JVMException(ex);
            } catch (IllegalArgumentException ex) {
               _logger.warn("Illegal constructor arguments  {}",args);
                throw new JVMException(ex);
            }
        } catch (Exception ex) {
            throw new JVMException(ex);
     ...
     * 
     * @throws JVMException, Exception
     */
    @SuppressWarnings("unchecked")
	public <T> boolean new_lambda_3(Term interfaceName, Term implementation, Term id)throws JVMException,Exception { 
    	try {
    		counter++;
    		...
    		Class<?> lambdaMetaFactory = Generator.make(
     ...
     * @throws JVMException
     */
    public boolean destroy_object_1(Term id) throws JVMException {
        id = id.getTerm();
        try {
            if (id.isGround()) {
     ...
     * @throws JVMException
     */
    public boolean java_class_4(Term clSource, Term clName, Term clPathes,Term id) throws JVMException {
     ...
     * @throws JVMException
     */
	public boolean new_class_4(Term clSource, Term clName, Term clPathes,Term id) throws JVMException {
		...
		try {
            ...
            Iterator<? extends Term> it = classPathes.iterator();
            String cp = "";
            while (it.hasNext()) {
                if (cp.length() > 0) {
                    cp += ";";
            ...
            try {
               ...
               _logger.warn("Compilation of java sources failed: Creation of {}.java failed",fullClassPath);
                throw new JVMException(ex);
            }
            ...
            try 
            {
               ...
               _logger.warn("Compilation of java sources failed: Java Class compiled, but not created: {}",fullClassName);
                throw new JVMException(ex);
            }
        } catch (Exception ex) {
            throw new JVMException(ex);
		...
		try {
			...
			if (!objId.isAtom()) {
				if (objId instanceof Var) {
					throw new JVMException(new IllegalArgumentException(objId
							.toString()));
				}
			...
			if (obj != null) {
				...
				if (m != null) {
					try {
						...
						_logger.warn("Method invocation failed: {} ( signature: {} )",methodName,args);
						throw new JVMException(ex);
					}
				} else {
                    _logger.warn("Method not found: {} ( signature: {} )", methodName, args);
            ...
            _logger.warn("Invalid arguments: {} ( method: {} )", args, methodName);
          	throw new JVMException(ex);
		} catch (Exception ex) {
            _logger.warn("Generic error in method invocation {}", methodName);
			throw new JVMException(ex);
		}
	}
     ...
     * 
     */
    public boolean set_classpath_1(Term paths) throws JVMException
    {
    	try {
    		paths = paths.getTerm();
        	if(!paths.isList())
        ...
        {
            _logger.warn("Illegal list of paths: {}", paths);
	...
	public boolean get_classpath_1(Term paths) throws JVMException
    {
    	try {
    		paths = paths.getTerm();
    ...
    private boolean java_set(Term objId, Term fieldTerm, Term what) {
        ...
        try {
            	...
            	if(clName != null)
            	{
            		try {
                       ...
                       _logger.warn("Java class not found: " + clName);
                        ...
                        _logger.warn("Static field {} not found in class {}", fieldName, Tools.removeApices(((Struct) objId).getArg(0).toString()));
                        return false;
                    }
            	}
            }
            ...
            if (what instanceof Number) {
                ...
                } else if (wn instanceof Double) {
                    field.setDouble(obj, wn.doubleValue());
                } else if (wn instanceof Long) {
                    field.setLong(obj, wn.longValue());
                } else if (wn instanceof Float) {
           ...
           _logger.warn("Field {} not found in class {}",fieldName,objId);
            return false;
        } catch (Exception ex) {
            return false;
        }
    ...
    private boolean java_get(Term objId, Term fieldTerm, Term what) {
        ...
        try {
            ...
            if (fc.equals(Integer.TYPE) || fc.equals(Byte.TYPE)) {
                ...
                return unify(what, new Long(value));
            } else if (fc.equals(java.lang.Float.TYPE)) {
                float value = field.getFloat(obj);
                return unify(what, new Float(value));
            } else if (fc.equals(java.lang.Double.TYPE)) {
                double value = field.getDouble(obj);
                return unify(what, new Double(value));
            } else {
    ...
    public boolean java_array_set_primitive_3(Term obj_id, Term i, Term what)
        ...
        Object obj = null;
        if (!index.isInteger()) {
            throw new JVMException(new IllegalArgumentException(index
                    .toString()));
        }
        try {
            Class<?> cl = null;
            ...
            if (name.equals("class [I")) {
                if (!(what instanceof Number)) {
                    throw new JVMException(new IllegalArgumentException(what
                            .toString()));
                }
     ...
     * @throws JVMException
     */
    public boolean java_array_get_primitive_3(Term obj_id, Term i, Term what) throws JVMException {
        Struct objId = (Struct) obj_id.getTerm();
        Number index = (Number) i.getTerm();
        what = what.getTerm();
        Object obj = null;
        if (!index.isInteger()) {
            throw new JVMException(new IllegalArgumentException(index.toString()));
        }
        try {
            Class<?> cl = null;
            String objName = Tools.removeApices(objId.toString());
            ...
            if (obj != null) {
                ...
                throw new JVMException(new IllegalArgumentException(objId.toString()));
            }
            ...
            if (name.equals("class [I")) {
                ...
                if (unify(what, value))
                    ...
                    throw new JVMException(new IllegalArgumentException(what.toString()));
            } else if (name.equals("class [D")) {
                Term value = new Double(Array.getDouble(obj,index.intValue()));
                if (unify(what, value))
                    return true;
                else
                    throw new JVMException(new IllegalArgumentException(what
                ...
                Term value = new Float(Array.getFloat(obj, index
                        .intValue()));
                if (unify(what, value))
                    return true;
                else
                ...
                Term value = new Long(Array.getLong(obj, index
                        .intValue()));
                if (unify(what, value))
                    return true;
                else
    ...
    private boolean parse_as(Object[] values, Class<?>[] types, int i,
        ...
        try {
                ...
                if (!castWhat_name.equals("null")) {
                    ...
                    if (obj_to_cast == null) {
                        ...
                        try {
                            ...
                            _logger.warn("Java class not found: {}",castTo_name);
                            return false;
                        }
                    }
                } else {
            ...
            _logger.warn("Casting {} to {} failed",castWhat,castTo);
            return false;
        }
        return true;
    }
     ...
     *             if the object id is not valid
     */
    public boolean register_1(Term id) throws JVMException
    {
    	id = id.getTerm();
    	Object obj =  null; 
    	try
        ...
        {
        	_logger.warn("Illegal object id {}", id.toString());
            throw new JVMException(e);
        }
    }
     ...
     *             if the object id is not valid
     */
    public boolean unregister_1(Term id) throws JVMException
    {
    	id = id.getTerm(); 
    	try
        {
