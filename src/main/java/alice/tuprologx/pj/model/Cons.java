...
public class Cons<H extends Term<?>, R extends Compound<?>> extends Compound<Cons<H,R>> implements Iterable<Term<?>> {
    ...
    */
    ...
    static <Z extends Cons<?,?>> Z unmarshal(alice.tuprolog.core.data.Struct s) {
        if (!matches(s))
            throw new UnsupportedOperationException();
    ...
    static boolean matches(alice.tuprolog.core.data.Term t) {
        return (!(t instanceof alice.tuprolog.core.data.Var) && t.isCompound() && !t.isList());
    }
    ...
    public alice.tuprolog.core.data.Struct marshal() {
        alice.tuprolog.core.data.Term[] termArray = new alice.tuprolog.core.data.Term[arity()];
        int i = 0;
        for (Term<?> t: this) {
            termArray[i++]=t.marshal();        
        }
        return new alice.tuprolog.core.data.Struct(_theName,termArray);
    }
    /*
