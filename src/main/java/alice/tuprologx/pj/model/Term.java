...
public abstract class Term<X extends Term<?>> {
    ...
    public abstract alice.tuprolog.core.data.Term marshal() /*{
            throw new UnsupportedOperationException();
        }*/;
        
    public static <Z extends Term<?>> Z unmarshal(alice.tuprolog.core.data.Term t) {
		if (Int.matches(t)) {
			// return (Z)Int.unmarshal((alice.tuprolog.core.data.numeric.Int)t);
			return uncheckedCast(Int.unmarshal((alice.tuprolog.core.data.numeric.Int)t));
		}
		else if (Double.matches(t)) {
			//return (Z)Double.unmarshal((alice.tuprolog.core.data.numeric.Double)t);
			return uncheckedCast(Double.unmarshal((alice.tuprolog.core.data.numeric.Double)t));
		}
        else if (JavaObject.matches(t)) {
			//return (Z)JavaObject.unmarshalObject((alice.tuprolog.Struct)t);
			return uncheckedCast(JavaObject.unmarshalObject((alice.tuprolog.core.data.Struct)t));
		}
		else if (Atom.matches(t)) {
			//return (Z)Atom.unmarshal((alice.tuprolog.Struct)t);
			return uncheckedCast(Atom.unmarshal((alice.tuprolog.core.data.Struct)t));
		}
		else if (Bool.matches(t)) {
			//return (Z)Bool.unmarshal((alice.tuprolog.Struct)t);
			return uncheckedCast(Bool.unmarshal((alice.tuprolog.core.data.Struct)t));
		}
		else if (List.matches(t)) {
			//return (Z)List.unmarshal((alice.tuprolog.Struct)t);
			return uncheckedCast(List.unmarshal((alice.tuprolog.core.data.Struct)t));
		}
        else if (JavaTerm.matches(t)) {
			//return (Z)JavaTerm.unmarshalObject((alice.tuprolog.Struct)t.getTerm());
			return uncheckedCast(JavaTerm.unmarshalObject((alice.tuprolog.core.data.Struct) t.getTerm()));
		}                
        else if (Cons.matches(t)) {
			//return (Z)Cons.unmarshal((alice.tuprolog.Struct)t);
			return uncheckedCast(Cons.unmarshal((alice.tuprolog.core.data.Struct)t));
		}                
        else if (Var.matches(t)) {
			//return (Z)Var.unmarshal((alice.tuprolog.Var)t);
			return uncheckedCast(Var.unmarshal((alice.tuprolog.core.data.Var)t));
		}
