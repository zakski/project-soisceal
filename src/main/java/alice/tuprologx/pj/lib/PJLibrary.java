	...
	public   boolean java_array_get_primitive_3(Term obj_id, Term i, Term what) {
		...
		try {
			...
			if (name.equals("class [I")){
				...
				Term value = new Float(Array.getFloat(obj,index.intValue()));
				return unify(what,value);
			}  else if (name.equals("class [L")){
				Term value = new alice.tuprolog.core.data.numeric.Long(Array.getLong(obj,index.intValue()));
				return unify(what,value);
			}  else if (name.equals("class [C")){
				Term value = new alice.tuprolog.core.data.Struct(""+Array.getChar(obj,index.intValue()));
				return unify(what,value);
			}  else if (name.equals("class [Z")){
				boolean b = Array.getBoolean(obj,index.intValue());
				if (b) {
					return unify(what,alice.tuprolog.core.data.Term.TRUE());
				} else {
					return unify(what,alice.tuprolog.core.data.Term.FALSE());
				}
			}  else if (name.equals("class [B")){
	...
	private boolean parseResult(Term id, Object obj) {
		...
		try {
			if (Boolean.class.isInstance(obj)) {
				...
				return unify(id, new Long(((java.lang.Long) obj).longValue()));
			} else if (java.lang.Float.class.isInstance(obj)) {
				return unify(id, new Float(((java.lang.Float) obj).floatValue()));
			} else if (java.lang.Double.class.isInstance(obj)) {
				return unify(id, new Double(((java.lang.Double) obj).doubleValue()));
