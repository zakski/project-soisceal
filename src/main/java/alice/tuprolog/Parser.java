...
import alice.tuprolog.core.data.util.TermIterator;
...
public class Parser implements /*Castagna 06/2011*/IParser,/**/ Serializable
{
	...
	private alice.tuprolog.core.parser.Tokenizer tokenizer;
	...
	public Parser(String theoryText, HashMap<Term, Integer> mapping) {		 
		tokenizer = new alice.tuprolog.core.parser.Tokenizer(theoryText);
		offsetsMap = mapping;		 
	}
	/**/
	...
	public Parser(InputStream theoryText) {
		tokenizer = new alice.tuprolog.core.parser.Tokenizer(new BufferedReader(new InputStreamReader(theoryText)));
	}
	...
	public Term nextTerm(boolean endNeeded) throws InvalidTermException {
		try {
			alice.tuprolog.core.parser.Token t = tokenizer.readToken();
			if (t.isEOF())
				return null;
			...
			if (endNeeded && tokenizer.readToken().getType() != alice.tuprolog.core.parser.Tokenizer.END())
				/*Castagna 06/2011*/
	...
	public static Term parseSingleTerm(String st, OperatorManager op) throws InvalidTermException {
		try {
			...
			alice.tuprolog.core.parser.Token t = p.tokenizer.readToken();
			if (t.isEOF())
	...
	private IdentifiedTerm exprB(int maxPriority, boolean commaIsEndMarker) throws InvalidTermException, IOException {
		...
		alice.tuprolog.core.parser.Token operator = tokenizer.readToken();
	...
	private IdentifiedTerm parseLeftSide(boolean commaIsEndMarker, int maxPriority) throws InvalidTermException, IOException {
		//1. prefix expression
		alice.tuprolog.core.parser.Token f = tokenizer.readToken();
	...
	private Term expr0() throws InvalidTermException, IOException {
		alice.tuprolog.core.parser.Token t1 = tokenizer.readToken();
		...
		*/
		...
		if (t1.isType(alice.tuprolog.core.parser.Tokenizer.ATOM()) || t1.isType(alice.tuprolog.core.parser.Tokenizer.SQ_SEQUENCE()) || t1.isType(alice.tuprolog.core.parser.Tokenizer.DQ_SEQUENCE())) {
			if (!t1.isFunctor())
			/*Castagna 06/2011*/
			{
			...
			alice.tuprolog.core.parser.Token t2 = tokenizer.readToken();   //reading left par
			if (!t2.isType(alice.tuprolog.core.parser.Tokenizer.LPAR()))
			...
			alice.tuprolog.core.parser.Token t3 = tokenizer.readToken();
			if (t3.isType(alice.tuprolog.core.parser.Tokenizer.RPAR()))      //reading right par
			/*Castagna 06/2011*/
			{
		...
		if (t1.isType(alice.tuprolog.core.parser.Tokenizer.LPAR())) {
			Term term = expr(false);
			if (tokenizer.readToken().isType(alice.tuprolog.core.parser.Tokenizer.RPAR()))
				return term;
			/*Castagna 06/2011*/
		...
		if (t1.isType(alice.tuprolog.core.parser.Tokenizer.LBRA())) {
			alice.tuprolog.core.parser.Token t2 = tokenizer.readToken();
			if (t2.isType(alice.tuprolog.core.parser.Tokenizer.RBRA()))
				return new Struct();

			tokenizer.unreadToken(t2);
			Term term = expr0_list();
			if (tokenizer.readToken().isType(alice.tuprolog.core.parser.Tokenizer.RBRA()))
				return term;
			/*Castagna 06/2011*/
		...
		if (t1.isType(alice.tuprolog.core.parser.Tokenizer.LBRA2())) {
			alice.tuprolog.core.parser.Token t2 = tokenizer.readToken();
			if (t2.isType(alice.tuprolog.core.parser.Tokenizer.RBRA2()))
			/*Castagna 06/2011*/
			{
				//return new Struct("{}");
				Term b = new Struct("{}");
	...
	static alice.tuprolog.core.data.numeric.Number parseInteger(String s) {
		long num = java.lang.Long.parseLong(s);
		if (num > Integer.MIN_VALUE && num < Integer.MAX_VALUE)
			return new alice.tuprolog.core.data.numeric.Int((int) num);
		else
			return new alice.tuprolog.core.data.numeric.Long(num);
	}

	static alice.tuprolog.core.data.numeric.Double parseFloat(String s) {
		double num = java.lang.Double.parseDouble(s);
		return new alice.tuprolog.core.data.numeric.Double(num);
	}

	static alice.tuprolog.core.data.numeric.Number createNumber(String s){
		try {
