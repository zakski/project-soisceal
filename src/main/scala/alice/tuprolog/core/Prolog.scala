 ...
 */
package alice.tuprolog.core

import java.{util => ju}

import alice.tuprolog.core.data.{Struct, Term}
import alice.tuprolog.core.engine.context.ExecutionContext
import alice.tuprolog.core.engine.{Engine, EngineManager, Solution}
import alice.tuprolog.core.event.interpreter.{LibraryEvent, QueryEvent, TheoryEvent}
import alice.tuprolog.core.event.io.OutputEvent
import alice.tuprolog.core.event.logging.{ExceptionEvent, SpyEvent, WarningEvent}
import alice.tuprolog.core.exception.interpreter._
import alice.tuprolog.core.flag.FlagManager
import alice.tuprolog.core.lib.{LibraryManager, PrimitiveManager}
import alice.tuprolog.core.theory.{Theory, TheoryManager}
import alice.tuprolog.event._
import alice.tuprolog.interfaces.IProlog
import alice.tuprolog.util.VersionInfo
import alice.tuprolog.util.exception.lib.InvalidLibraryException
import alice.tuprolog.util.exception.solve.{MalformedGoalException, NoMoreSolutionsException}
import alice.tuprolog.{Library, Operator, OperatorManager, Parser}
...
class Prolog(spy: Boolean, warning: Boolean) extends IProlog with Serializable {
  ...
  def this() {
    ...
    try {
      if (System.getProperty("java.vm.name") == "IKVM.NET") loadLibrary("OOLibrary.OOLibrary, OOLibrary")
      else loadLibrary("alice.tuprolog.lib.OOLibrary")
    }
    catch {
      case ex: Exception => {
        ex.printStackTrace
   ...
   * @see alice.tuprolog.core.theory.Theory
   */
