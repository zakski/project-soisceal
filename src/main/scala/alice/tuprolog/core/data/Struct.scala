...
import alice.tuprolog.core.data.util.StructIterator
import alice.tuprolog.core.lib.PrimitiveInfo
...
class Struct(name : String, arity : Int) extends Term with Iterable[Term] {
   ...
   */
  private[tuprolog] def this(f: String, al: ju.LinkedList[Term]) {
   ...
   */
  private[tuprolog] def setArg(index: Int, argument: Term) {
    _arg(index) = argument
  }
   ...
   */
  private[tuprolog] def resolveTerm(vl: ju.LinkedList[Var], count: Long): Long = {
   ...
   */
  private[tuprolog] def toListStruct: Struct = {
   ...
   */
  private[tuprolog] def fromList: Struct = {
   ...
   */
  private[tuprolog] def insert(t: Term) {
   ...
   */
  private[tuprolog] def setPrimitive(b: PrimitiveInfo) {
    primitive = b
  }
  ...
  private[tuprolog] override def toStringAsArg(op: OperatorManager, prio: Int, x: Boolean): String = {
    var p: Int = 0
    var v: String = ""
    if ((_name == ".") && _arity == 2) {
