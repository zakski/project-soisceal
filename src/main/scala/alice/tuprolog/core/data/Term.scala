...
import alice.tuprolog.{Parser, OperatorManager}
...
abstract class Term extends Serializable {
   ...
   */
  private[tuprolog] def resolveTerm(count: scala.Long): scala.Long
   ...
   */
  private[tuprolog] def copy(vMap: ju.AbstractMap[Var, Var], idExecCtx: Int): Term

  /**
   * gets a copy for result.
   */
  private[tuprolog] def copy(vMap: ju.AbstractMap[Var, Var], substMap: ju.AbstractMap[Term, Var]): Term
   ...
   */
  private[tuprolog] def unify(varsUnifiedArg1: ju.List[Var], varsUnifiedArg2: ju.List[Var], t: Term): Boolean

  /**
   * Gets the string representation of this term
   * as an X argument of an operator, considering the associative property.
   */
  private[tuprolog] def toStringAsArgX(op: OperatorManager, prio: Int): String = {
   ...
   */
  private[tuprolog] def toStringAsArgY(op: OperatorManager, prio: Int): String = {
   ...
   */
  private[tuprolog] def toStringAsArg(op: OperatorManager, prio: Int, x: Boolean): String = {
    toString
  }
