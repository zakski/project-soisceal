...
abstract class Number extends Term with Comparable[Number] {
   ...
   */
  private[tuprolog] override def copy(vMap: ju.AbstractMap[Var, Var], idExecCtx: scala.Int): Term = this

  /**
   * gets a copy of the term.
   */
  private[tuprolog] override def copy(vMap: ju.AbstractMap[Var, Var], substMap: ju.AbstractMap[Term, Var]): Term = this


  private[tuprolog] override def resolveTerm(count: scala.Long): scala.Long = count
