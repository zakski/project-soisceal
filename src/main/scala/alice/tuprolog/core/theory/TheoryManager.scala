 ...
 */
package alice.tuprolog.core.theory
...
import alice.tuprolog.core.engine.subgoal.tree.SubGoalLeaf
import alice.tuprolog.core.exception.interpreter.{InvalidTermException, InvalidTheoryException}
import alice.tuprolog.core.theory.clause.{ClauseInfo, FamilyClausesList, ClauseDatabase}
...
class TheoryManager(vm: Prolog) extends Serializable {
    ...
    synchronized {
      ...
      if (dyn) {
        ...
        if (_staticDBase.containsKey(key)) {
          _engine.warn("A static predicate with signature " + key + " has been overriden.")
        }
      }
  ...
  private def runDirective(c: Struct): Boolean = {
    if (("':-'" == c.getName) || (":-" == c.getName) && c.getArity == 1 && c.getTerm(0).isInstanceOf[Struct]) {
      ...
      try {
        if (!_primitiveManager.evalAsDirective(dir)) _engine.warn("The directive " + dir.getPredicateIndicator + " is unknown.")
      }
      catch {
        case t: Throwable => {
          _engine.warn("An exception has been thrown during the execution of the " + dir.getPredicateIndicator + " directive.\n" + t.getMessage)
        }
      }
      return true
    }
