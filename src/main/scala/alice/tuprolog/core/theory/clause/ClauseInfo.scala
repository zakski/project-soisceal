...
import alice.tuprolog.core.engine.subgoal.tree.{SubGoalLeaf, SubGoalTree}
...
object ClauseInfo {
  ...
  /* TODO renable private[tuprolog]*/ def extractBody(body: Term): SubGoalTree = {
...
class ClauseInfo protected(theClause : Struct, theHead : Struct, theBody : SubGoalTree, theLib : String) {
  ...
  protected[tuprolog] def getLib = libName

  protected[tuprolog] def getHead= head

  protected[tuprolog] def getBody =body

  protected[tuprolog] def getClause = clause

  private[tuprolog] def performCopy(theExecCtx: Int) = {
  ...
  protected[tuprolog] def getHeadCopy: Struct = {
     headCopy
  }

  protected[tuprolog] def getBodyCopy: SubGoalTree = {
    bodyCopy
  }
