 ...
 */
@SerialVersionUID(1L)
class Theory private[tuprolog]() extends Serializable {
  private var theory: String = ""
  private var clauseList: Struct = null
  ...
  def iterator(engine: core.Prolog): Iterator[_ <: Term] = {
    if (isTextual)
   ...
   *
   */
  private[tuprolog] def isTextual: Boolean = {
    theory != null
  }

  private[tuprolog] def getClauseListRepresentation: Struct = {
    clauseList
  }
