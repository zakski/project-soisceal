...
import alice.tuprolog.core.data.util.TermQueue
import alice.tuprolog.core.engine.context.ExecutionContext
import alice.tuprolog.core.engine.state._
import alice.tuprolog.core.engine.subgoal.tree.SubGoalTree
import alice.tuprolog.core.lib.{LibraryManager, PrimitiveManager}
import alice.tuprolog.core.theory.TheoryManager
...
class EngineRunner(vm: Prolog, id: scala.Int) extends java.io.Serializable with Runnable {
  ...
  /* Current environment */
  private[tuprolog] var env: Engine = null
   ...
   * States
   */
  private[tuprolog] final val INIT: State = new InitState(this)
  private[tuprolog] final val GOAL_EVALUATION: State = new GoalEvaluationState(this)
  private[tuprolog] final val EXCEPTION: State = new ExceptionState(this)
  private[tuprolog] final val RULE_SELECTION: State = new RuleSelectionState(this)
  private[tuprolog] final val GOAL_SELECTION: State = new GoalSelectionState(this)
  private[tuprolog] final val BACKTRACK: State = new BacktrackState(this)
  private[tuprolog] final val END_FALSE: State = new EndState(this, EngineRunner.FALSE)
  private[tuprolog] final val END_TRUE: State = new EndState(this, EngineRunner.TRUE)
  private[tuprolog] final val END_TRUE_CP: State = new EndState(this, EngineRunner.TRUE_CP)
  private[tuprolog] final val END_HALT: State= new EndState(this, EngineRunner.HALT)

  private[tuprolog] def spy(action: String, env: Engine) {
    mediator.spy(action, env)
  }

  private[tuprolog] def warn(message: String) {
    mediator.warn(message)
  }

  private[tuprolog] def exception(message: String) {
  ...
  private[tuprolog] def find(t: Term): List[ClauseInfo] = {
    return theoryManager.find(t)
  }

  private[tuprolog] def identify(t: Term) {
    primitiveManager.identifyPredicate(t)
  }

  private[tuprolog] def pushSubGoal(goals: SubGoalTree) {
    env.context.goalsToEval.pushSubGoal(goals)
  }

  private[tuprolog] def cut {
    env.cut(env.context.choicePointAfterCut)
  }

  private[tuprolog] def getCurrentContext: ExecutionContext = {
   ...
   */
  private[tuprolog] def hasOpenAlternatives: Boolean = {
    if (sinfo == null)
      return false
   ...
   */
  private[tuprolog] def isHalted: Boolean = {
  ...
  private[tuprolog] def getTheoryManager: TheoryManager = {
