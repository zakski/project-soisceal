...
class ExceptionState(runner : EngineRunner) extends State(runner,"Exception") {
  private[tuprolog] final val catchTerm: Term = Term.createTerm("catch(Goal, Catcher, Handler)")
  private[tuprolog] final val javaCatchTerm: Term = Term.createTerm("java_catch(Goal, List, Finally)")
