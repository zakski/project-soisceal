...
import alice.tuprolog.util.exception.lib.{LibraryNotFoundException, LibraryLoadException, InvalidLibraryException}
    ...
    try {
      if (System.getProperty("java.vm.name") == "Dalvik") {
        ...
        */
        ...
        if (!(System.getProperty("java.vm.name") == "IKVM.NET")) {
