 ...
 */
package alice.tuprolog.core.lib
...
import alice.tuprolog.interfaces.{IPrimitiveManager, IPrimitives}
import alice.tuprolog.{BuiltIn, Library}
...
class PrimitiveManager(vm: Prolog) extends IPrimitiveManager {
  ...
  private[tuprolog] def createPrimitiveInfo(src: IPrimitives) {
  ...
  private[tuprolog] def deletePrimitiveInfo(src: IPrimitives) {
  ...
  private[tuprolog] def getLibraryDirective(name: String, nArgs: Int): Library = {
    try {
  ...
  private[tuprolog] def getLibraryPredicate(name: String, nArgs: Int): Library = {
    try {
  ...
  private[tuprolog] def getLibraryFunctor(name: String, nArgs: Int): Library = {
    try {
