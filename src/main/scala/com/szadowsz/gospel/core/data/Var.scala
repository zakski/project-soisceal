...
@SerialVersionUID(1L)
object Var {
  private[gospel] val ANY: String = "_"
  private[gospel] val ORIGINAL: Int = -1
  private[gospel] val PROGRESSIVE: Int = -2
...
final class Var extends Term {
   ...
   */
  private[gospel] def rename(idExecCtx: Int, count: Int) {
    id = idExecCtx
    if (id > -1) {
   ...
   */
  private[gospel] def setLink(l: Term) {
    link = l
  }
   ...
   * Set the timestamp
   */
  private[gospel] def setTimestamp(t: Long) {
    timestamp = t
  }
