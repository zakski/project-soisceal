...
import com.szadowsz.gospel.core.engine.Result
...
/**
  ...
  */
@SerialVersionUID(1L)
class Solution(q: Term, g: Struct, result: Result, resultVars: util.List[Var]) extends Serializable {
  private val query: Term = q
  private val goal: Struct = g
  private val endState: Result = result
  private val bindings: util.List[Var] = resultVars
  
  def this(initGoal: Term) {
    this(initGoal, null, Result.FALSE, new util.ArrayList[Var]())
  }
  ...
  /**
    ...
    */
  def isSuccess: Boolean = endState.ordinal() > Result.FALSE.ordinal()
  ...
  /**
    ...
    * @return true if the solve was successful
    */
  def isHalted: Boolean = endState == Result.HALT
  ...
  /**
    ...
    * @return true if the solve was successful
    */
  def hasOpenAlternatives: Boolean = endState == Result.TRUE_CP
  ...
  def getVarOpt(varName: String): Option[Term] = {
    if (isSuccess) bindings.asScala.find(v => v.getName == varName).map(_.getBinding) else None
  }
  ...
  override def toString: String = {
    result match {
      case Result.TRUE | Result.TRUE_CP =>
        ...
        for (v <- bindings.asScala){
          if (!v.isAnonymous && v.isBound && !v.isBindingAnonymous) {
            st.append(v).append("  ")
          }
        }
        st.toString.trim
      case Result.HALT => "halt."
      case Result.FALSE => "no."
    }
  }
}