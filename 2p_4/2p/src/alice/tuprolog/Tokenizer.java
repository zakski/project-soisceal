...
public class Tokenizer extends StreamTokenizer implements Serializable 
{
    ...
    Token readNextToken() throws IOException, InvalidTermException 
    {
        ...
        if (typea == TT_WORD)
        {
            ...
            {                                         	    // otherwise, it must be an atom (or wrong)
            	/*Castagna 06/2011*/
        ...
        if (typea == '\'' || typea == '\"' || typea == '`' )
        {
            ...
            while (true) 
            {   // run through entire quote and added body to quote buffer
            	/*Castagna 06/2011*/
                //typea = super.nextToken();
            	typea = tokenConsume();
...
//            }
        ...
        {
            try 
            { // the various parseInt checks will throw exceptions when parts of numbers are written illegally
                ...
                if (typeb != '.' && typeb != '\'') 
                { // i.e. not float or character constant
                	 /*Castagna 06/2011*/
                ...
                if (typec != TT_WORD) 
                { // if its not, the period is an End period
                	/*Castagna 06/2011*/
                ...
                if (exponent >= 1) 
                {                                  // the float must have a valid exponent
                    if (exponent == svalc.length() - 1)
                    {             // the exponent must be signed exponent
                    	/*Castagna 06/2011*/
