...
//        System.out.println("STATE END GOALLLLLLL: "+e.query); 
    ...
    {
	    if(c.getEngineMan().getRelinkVar())
	    {   //devo fare il relink solo se ho var libere nella bagof E SE NON CI SONO ESISTENZE
...
//	    	}
	    	...
	    	if(((Struct)query).getArity()>2 && ((Struct)query).getArg(2) instanceof Struct)
	    	{ //casi in cui ci potrebbe essere problema di soluzione
		    	...
		    	while (tgoal instanceof Var && ((Var)tgoal).getLink()!=null)
		    	{
		    		...
		    		if(tgoal instanceof Struct)
		    		{
		    			...
		    			if(p.unify(tgoal, ((Var)initBag).getLink()))
		    			{
		    				//In realta' sembra che gia' qui non unifichi
		    	...
		    	for (int i=0; i<bag.size();i++) 
		    	{
		    		...
		    		for(int y=0; y<a.length;y++)
		    		{
		    			...
		    			if(vv_link.isEqual(t))
		    			{
		    				...
		    				{
		    					substituteVar((Struct)vv_link.getLink(),lSolVar,lgoalBOVar);
