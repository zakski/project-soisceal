package  alice.tuprolog.ios.LPaaS;
...
import alice.tuprolog.ios.IOSViewController;

public class LPaaS_Manager implements I_LPaaS_Manager
{
	...
	public LPaaS_Manager(IOSViewController viewController)
	{
	...
	private void URLResponse(String url) 
	{
		...
		LPaaS_Manager.currentUrl = new NSURL(URL);
		...
		DispatchQueue.getMainQueue().async(new Runnable() 
		{
	        ...
	        public void run() 
	        {
				UIApplication.getSharedApplication().openURL(LPaaS_Manager.currentUrl);
	        }
	    });
	}
}