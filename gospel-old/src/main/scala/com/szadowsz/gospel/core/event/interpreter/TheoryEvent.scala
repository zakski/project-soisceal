 ...
 */
@SerialVersionUID(1L)
final class TheoryEvent(source: PrologEngine, oldth: Theory, newth: Theory) extends PrologEvent(source) {
  private val old = oldth
  private val current = newth
   ...
   */
  def getNewTheory : Theory = current
}
