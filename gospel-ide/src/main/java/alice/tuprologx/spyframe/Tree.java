...
public class Tree<T> extends JComponent {
    ...
    private static final Font defaultfont = new Font(Font.SANS_SERIF, Font.PLAIN, 12);
    private static final long serialVersionUID = 1L;
    /**
     * The totree to consult for constructing the tree.
     */
    private final ToTree<T> totree;
    /**
     ...
     *               not be changed.
     */
    private void setViewParams(float xspace, float yspace, float border, Font f) {
        this.xspace = xspace;
        this.yspace = yspace;
        this.border = border;
        if (f != null) font = f;
     ...
     */
    private float pose(Node node, float xpos, int level) {
     ...
     */
    private void paintTree(Graphics2D g2, Node node) {
