...
public class ConsoleDialog
    ...
    private final Console consoleManager;
    ...
    void enableStopButton(boolean flag) {
        bStop.setEnabled(flag);
    }

    void enableSolutionCommands(boolean flag) {
        bNext.setEnabled(flag);
        bAccept.setEnabled(flag);
    }

    private void enableTheoryCommands(boolean flag) {
    ...
    private String[] getVariablesName(Object[] querySolutions) {
        ...
        if (columns > 0) {
            for (Object querySolution : querySolutions) {
                if (getVariablesNumber(((QueryEvent) querySolution).getSolution()) == getVariablesNumber(querySolutions)) {
                    return getVariablesName(((QueryEvent) querySolution).getSolution());
                }
            }
    ...
    private int getVariablesNumber(Object[] querySolutions) {
        int count = 0;
        for (Object querySolution : querySolutions) {
            int n = getVariablesNumber(((QueryEvent) querySolution).getSolution());
            if (count < n)
                count = n;
        }
        return count;
            ...
            if (selectDisplayModality == 1 || selectDisplayModality == 2) {
                ...
                if (selectDisplayModality == 2) {
                    ...
                    for (int var = 0; var < variables.length; var++)
                        for (int i = 2 * var; i < tableModelList.size(); ) {
    ...
    /*Castagna 06/2011*/
    private void setExceptionEnabled(boolean enable) {
        exception.setText("");
