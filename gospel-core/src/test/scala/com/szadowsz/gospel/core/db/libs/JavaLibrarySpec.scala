...
class JavaLibrarySpec extends FunSpec with BaseEngineSpec {
  ...
  describe("Java library basics") {
    ...
    it("should retrieve Dynamic Objects Successfully") {
      ...
      val id = info.getVarValue("Obj").asInstanceOf[Struct]
    ...
    it("should pass Java Object test #1 Successfully") {
      ...
      val result = info.getVarValue("Value").asInstanceOf[Number]
      ...
      info.isSuccess shouldBe true
      val result2 = info.getVarValue("StringValue").toString.replace("'", "")
    ...
    it("should pass Java Object test #2 Successfully") {
      ...
      val result = info.getVarValue("Res").asInstanceOf[Number]
    ...
    it("should pass java_call/3 test Successfully") {
      ...
      info.isSuccess shouldBe true
      val result = info.getVarValue("StringValue").toString.replace("'", "")
      ...
      info.isSuccess shouldBe true
      info.getVarValue("Res").toString.toInt shouldBe 0
      ...
      info.isSuccess shouldBe true
      info.getVarValue("Val").toString.toInt shouldBe 5
    }
    ...
    it("should pass java_array test Successfully") {
      ...
      var info = prolog.solve("demo(Value).")
      info.isSuccess shouldBe true
      val resultInt: Number = info.getVarValue("Value").asInstanceOf[Number]
      ...
      info.isSuccess shouldBe true
      val resultInt2: Number = info.getVarValue("Value").asInstanceOf[Number]
    ...
    it("should pass get_classpath test Successfully") {
      ...
      var info = prolog.solve("demo(Value).")
      info.isSuccess shouldBe true
      info.getTerm("Value").isList shouldBe true
      info.getTerm("Value").toString shouldBe "[]"
      setPath(true)
      ...
      info.isSuccess shouldBe true
      info.getTerm("Value").isList shouldBe true
      info.getTerm("Value").toString shouldBe "[" + paths + "]"
    }
    ...
    it("should pass register/1 test Successfully") {
      ...
      val obj: String = info.getTerm("R").toString
      val info2: Solution = prolog.solve("demo2(" + obj + ", V).")
      info2.isSuccess shouldBe true
      info2.getVarValue("V").toString.toInt shouldBe 3
    ...
    it("should pass unregister/1 test Successfully") {
      ...
      val id: Struct = info.getTerm("Res").asInstanceOf[Struct]
  ...
  describe("java_throw/1 and java_catch/3") {
    ...
    it("the manager should be executed with the substitutions made between the exception and the catcher, and then the finally should be executed") {
      ...
      val cause = info.getTerm("Cause").asInstanceOf[Int]
      ...
      val z = info.getTerm("Z").asInstanceOf[Int]
    ...
    it("should use the correct catcher in the list") {
      ...
      val x = info.getTerm("X")
      x.isInstanceOf[Var] shouldBe true

      val y = info.getTerm("Y")
